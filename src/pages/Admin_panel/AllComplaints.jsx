...
const AllComplaints = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [complaintsRes, officersRes] = await Promise.all([
        axios.get(`${API_URL}/api/admin/complaints`, axiosConfig),
        axios.get(`${API_URL}/api/admin/complaints/officers/workload`, axiosConfig),
      ]);
  ...
  const handlePriorityChange = async (complaintId, newPriority) => {
    try {
      ...
      await axios.put(
        `${API_URL}/api/admin/complaints/${complaintId}/priority`,
  ...
  const handleOpenPreview  = (url) => { setPreviewUrl(url); setOpenPreview(true); };
  ...
  return (
    <Box sx={{ p: { xs: 1, md: 3 } }}>
        ...
        {[
          ...
          <Grid item xs={12} sm={4} key={item.label}>
            <div style={{ background: "var(--surface)", padding: "1.5rem", borderRadius: "24px", border: "1px solid var(--border-soft)", boxShadow: "var(--card-shadow)", display: "flex", alignItems: "center", gap: "1.25rem" }}>
              ...
              <div>
                <Typography variant="body2" fontWeight="600" color="var(--text-muted)" sx={{ textTransform: "uppercase", letterSpacing: "0.05em" }}>{item.label}</Typography>
      ...
      {/* Filters */}
      <div style={{ background: "var(--surface)", padding: "1.5rem", borderRadius: "24px", border: "1px solid var(--border-soft)", display: "flex", flexWrap: "wrap", gap: "1rem", alignItems: "center", marginBottom: "2rem", boxShadow: "0 4px 20px rgba(0,0,0,0.03)" }}>
        <div style={{ position: "relative", flex: 1, minWidth: "280px" }}>
          ...
          <input
            ...
            style={{ width: "100%", padding: "0.85rem 1rem 0.85rem 3rem", borderRadius: "16px", border: "1px solid var(--border)", background: "rgba(0,0,0,0.02)", fontSize: "1rem", outline: "none", color: "var(--text-primary)" }}
          />
        </div>
        <div style={{ display: "flex", gap: "1rem", alignItems: "center" }}>
          <Filter size={20} color="var(--text-muted)" />
          <TextField select size="small" label="Department Search" value={selectedDept} onChange={(e) => setSelectedDept(e.target.value)} sx={{ width: 220, "& .MuiOutlinedInput-root": { borderRadius: "14px" } }}>
            ...
            <Table>
              ...
              <TableBody>
                {filteredComplaints.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={7} sx={{ py: 10, textAlign: "center", color: "var(--text-muted)" }}>No matching complaints found.</TableCell>
                  </TableRow>
                  ...
                  <TableRow key={c.id} hover sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                    ...
                    <TableCell>
                      <TextField select size="small" value={c.status} onChange={(e) => handleStatusChange(c.id, e.target.value)}
                        sx={{ width: 130, "& .MuiOutlinedInput-root": { borderRadius: "10px", fontWeight: "700", color: "#fff9f5", backgroundColor: c.status === "PENDING" ? "#eddf28f5" : c.status === "IN_PROGRESS" ? "#287ce9" : c.status === "RESOLVED" ? "#0fb449" : "#ef1414" } }}>
                        {["PENDING", "IN_PROGRESS", "RESOLVED", "REOPENED"].map((s) => <MenuItem key={s} value={s}>{s}</MenuItem>)}
                      </TextField>
                    </TableCell>

                    <TableCell>
                      <TextField select size="small" value={c.priority || "MEDIUM"} onChange={(e) => handlePriorityChange(c.id, e.target.value)}
                        sx={{ width: 120, "& .MuiOutlinedInput-root": { borderRadius: "10px", fontWeight: "700", color: "#fff9f5", backgroundColor: c.priority === "LOW" ? "#166534" : c.priority === "MEDIUM" ? "#f57636" : "#991b1b" } }}>
                    ...
                    <TableCell>
                      {c.assignedOfficer ? (
                        <div style={{ display: "flex", flexDirection: "column" }}>
                          ...
                          <span style={{ fontSize: "11px", color: c.assignedOfficer.status === "AVAILABLE" ? "#10b981" : "#ef4444", fontWeight: "700" }}>● {c.assignedOfficer.status}</span>
                        </div>
                    ...
                    <TableCell>
                      {c.officerEvidenceUrl ? (
                        <div style={{ position: "relative", width: "60px", height: "45px", cursor: "pointer" }} onClick={() => handleOpenPreview(`${API_URL}${c.officerEvidenceUrl}`)}>
                          <img src={`${API_URL}${c.officerEvidenceUrl}`} alt="Evidence" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "8px" }} />
                    ...
                    <TableCell align="right">
                      {(!c.assignedOfficer && selectedDept && c.category === selectedDept) ? (
                        <Button variant="contained" size="small" disabled={!availableOfficers.length} onClick={() => handleAssign(c.id, availableOfficers[0].id)}
                          sx={{ borderRadius: "10px", textTransform: "none", fontWeight: "700", background: "linear-gradient(135deg, var(--primary), var(--primary-strong))" }}>
                          Assign Auto
                        </Button>
                      ) : (
                        <IconButton size="small" onClick={() => handleOpenPreview(c.imageUrl ? `${API_URL}${c.imageUrl}` : "https://via.placeholder.com/400")}>
      ...
      <Dialog open={openPreview} onClose={handleClosePreview} maxWidth="md" PaperProps={{ sx: { borderRadius: "24px", overflow: "hidden" } }}>
        <DialogContent sx={{ position: "relative", p: 0 }}>
          <IconButton sx={{ position: "absolute", top: 12, right: 12, zIndex: 10, bgcolor: "rgba(0,0,0,0.5)", color: "white", "&:hover": { bgcolor: "rgba(0,0,0,0.7)" } }} onClick={handleClosePreview}>
            <CloseIcon />
          </IconButton>
